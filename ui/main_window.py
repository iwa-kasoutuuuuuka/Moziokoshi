...
class TranscriptionThread(QThread):
    ...
    def __init__(self, files, output_dir, mode, formats, options):
        ...
        self.enable_speculative = options.get('enable_speculative', True)
    ...
    def run(self):
        try:
            ...
            transcriber = Transcriber(self.mode, models_dir, app_dir, ffmpeg_path, enable_speculative=self.enable_speculative)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.chk_replace = QCheckBox("カスタム置換 (replacement_dict.txt)")
        self.chk_speculative = QCheckBox("推論支援 (Speculative Decoding) - 高速化")
        self.chk_replace.setChecked(True)
        self.chk_speculative.setChecked(True)
        self.chk_speculative.setToolTip("Assistantモデル(tiny)を使用して、largeモデルの推論を高速化します。")
        opt_layout.addWidget(self.chk_filler)
        opt_layout.addWidget(self.chk_punc)
        opt_layout.addWidget(self.chk_replace)
        opt_layout.addWidget(self.chk_speculative)
    ...
    def start_processing(self):
        ...
        opts = {
            ...
            'enable_speculative': self.chk_speculative.isChecked()
        }
